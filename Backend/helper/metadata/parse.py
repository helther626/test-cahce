...
def parse_media_name(name: str) -> dict:
    ...
    #----- existing PTN/GuessIt logic so legacy formats and explicit TMDB/IMDb
    #----- ids continue to be parsed exactly as before.
            ...
            if parsed["season"] is None and parsed["episode"] is None:
                ...
                if has_anchor:
                    ...
                    if g_season is not None:
                        try: g_season_int = int(g_season)
                        except (TypeError, ValueError): g_season_int = None
                        if g_season_int is not None and g_season_int > 0: parsed["season"] = g_season_int
...
_RES_BARE_TRAILING_RE = re.compile(r"(?i)(?<![\w])(?:240|360|480|576|720|1080|1440|2160|4320)(?![\w])(?=(?:[\s._-]*(?:\.[a-z0-9]{2,4})?)$)")
_QUALITY_TOKEN_RE = re.compile(r"(?i)(?:\d{3,4}x\d{3,4}|web-?dl|blu-?ray|bluray|hdtv|hdrip|webrip|bdrip|brrip|x264|x265|h\.?264|h\.?265|hevc|avc|aac|(?:ddp|dd\+?|e?ac-?3|dts(?:-?hd)?|truehd|atmos)\s*\d?(?:[\s.]\d)?|(?<!\d)\d[\s.]\d(?!\d)|10bit|8bit|multi(?:\s*audio)?|dual(?:\s*audio)?|esub|subs?|softsubs?|hardsubs?|(?<![\w])(?:bd|remux|encode)(?![\w]))")
...
def extract_absolute_episode(filename: str, parsed: dict | None = None) -> int | None:
    parsed = parsed or {}
    try:
        if parsed.get("season") is not None and int(parsed.get("season")) > 0: return None
    except (TypeError, ValueError):
        if parsed.get("season") is not None: return None
    if _SEASON_EP_RE.search(filename or ""): return None
    ep = parsed.get("episode")
    if isinstance(ep, list): return None
    if ep is not None:
        try: return int(ep)
    ...
    if prefixed: return int(prefixed[-1])
    bare = re.findall(r"(?:^|\s)(0*\d{1,4})(?:\s|$)", cleaned)
    candidates = []
    for x in bare:
        try: n = int(x)
        except ValueError: continue
        if n >= 1: candidates.append(n)
    if not candidates: return None
    long = [n for n in candidates if n >= 100]
    return long[-1] if long else candidates[-1]
...
def clean_anime_search_title(title: str, absolute_ep: int | None = None) -> str:
    ...
    if absolute_ep is not None:
        t = re.sub(rf"(?i)\s*[-–—]?\s*(?:e|ep|episode)?\s*0*{int(absolute_ep)}\s*$", "", t).strip()
...
def is_absolute_episode(parsed: dict, filename: str = "") -> bool:
    try:
        if parsed.get("season") is not None and int(parsed.get("season")) > 0: return False
    except (TypeError, ValueError):
        if parsed.get("season") is not None: return False
    if _SEASON_EP_RE.search(filename or ""): return False
    if parsed.get("episode") is not None and not isinstance(parsed.get("episode"), list): return True
...
def analyze_metadata_failure(filename: str) -> str:
    ...
    try: parsed = parse_media_name(parse_target)
    except Exception: return "The file name / caption could not be parsed. Give it a clear name like 'Movie Name (2021) 1080p'."
    ...
    title, season, episode, quality = parsed.get("title"), parsed.get("season"), parsed.get("episode"), parsed.get("quality")
